...
class HereroNewPred(nn.Module):
    def forward(self, graph, h, x, etype):
        ...
        # 一次性为所有节点类型的 'h'赋值
            ...
            _dotx = graph.edges[etype].data['score']

            # 一次性为所有节点类型的 'h'赋值
            graph.ndata['h'] = h  
            # 使用 点积 计算GNN模型训练后的特征的Dot
            graph.apply_edges(fn.u_dot_v('h', 'h', 'score'), etype=etype)

            _doth = graph.edges[etype].data['score']
            
            #返回结果，即为拼接后的特征
            return torch.cat((_dotx, _doth), dim=1)
        
#单层Dot+DNN+Sigmoid()运算
class LinkModel_s_new(nn.Module):
    def __init__(self,in_feats,hid_feats,out_feats,rel_names):
        
        super().__init__()
        self.rgcn = R_GCN(in_feats,hid_feats,out_feats,rel_names)
        self.pred = HereroNewPred()
        self.fc = nn.Linear(2, 1)
        self.predict = nn.Sigmoid()
    
    def forward(self, hg, x, etype):
        
        # x代表的是整个图的初始特征-node_features。
        h = self.rgcn(hg, x)
        
        dot_score = self.pred(hg, h, x, etype)
        
        linear_out = self.fc(dot_score)
        
        return self.predict(linear_out)
